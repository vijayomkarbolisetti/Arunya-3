...
const CentralParkPage = () => {
  ...
  useGSAP(
    () => {
      ...
      // Attraction Panels Scroll Animation
      const attractionPanels = gsap.utils.toArray(".attraction-panel");

      attractionPanels.forEach((panel) => {
        gsap.fromTo(
          panel,
          {
            opacity: 0,
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          <div className="space-y-48 md:space-y-64">
            {attractions.map((attraction, index) => (
              <div
                key={attraction.id}
                ref={(el) => (attractionsRef.current[index] = el)}
